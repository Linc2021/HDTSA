...
#' stationary time series consisting of \eqn{q\ (\geq 1)} both contemporaneously and serially uncorrelated subseries. then permutation step 
#' can grouping the components of a multivariate series \eqn{{\bf x}_t} into \eqn{q} groups.
#'
#'
#'
#' @details
...
#'  When \eqn{p} is large (\eqn{>n}), it is necessary to use the package \pkg{clime} to estimate the precision matrix \eqn{\widehat{{\bf V}}^{-1}} 
#'  where \eqn{\widehat{{\bf V}}} is a consistent estimator for \eqn{\mathrm{Var}({\bf y}_t)}. In segment step, when \eqn{p} is large (\eqn{p=O(n^{1/2}})), 
#'  it is recommended to use the thresholding method to calculate \eqn{\widehat{{\bf W}}_y}, see more information in Chang et al. (2018).
#'
#'
#'
#'
#' @param Y  \eqn{{\bf Y} = \{{\bf y}_1, \dots , {\bf y}_n \}'}, a data matrix with \eqn{n} rows and \eqn{p} columns, where \eqn{n} is the sample size and 
#'           \eqn{p} is the dimension of the time series.
...
#'          specified, then default constant \eqn{10*log10(n/p)}
#'          will be used.
...
#' \item{Pvalues}{Pvalue for multiple test: \deqn{H_0:\rho_{i,j}(h)=0\ \mathrm{\ for\ any}\ h\ =\ 0,\pm 1 ,\pm2,\dots,\pm m} 
#'               for each of \eqn{p(p-1)/2} pairs in ascending order if \code{permutation = 'fdr'}, see more information in Chang et al. (2018).}
#' \item{NoConnectPairs}{Number of connected pairs}
#' \item{Xpre}{The prewhitened data with \eqn{n-R} rows and \eqn{p} columns}
#'
#' @note The first step is transform the time series. It calculate linear transformation of the \eqn{p}-variate time series (or volatility processes)
#'       \eqn{y_t} such that the transformed series \eqn{x_t=By_t} is segmented into several lower-dimensional subseries. The second step is grouping 
#'       the transformed time series, The permutation is determined by grouping the components of a multivariate series X into \eqn{q} groups, where 
#'       \eqn{q} and the cardinal numbers of those groups are also unknown.
#'
#'
